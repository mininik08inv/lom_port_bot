...
#     db_conn = get_db_connection()
#     with db_conn.cursor() as cur:
#         sql_query = f"SELECT * FROM points_location WHERE abbreviation='{item}'"
#         # Делаем запрос к БД
#         try:
...
#             return res_data
#         except:
#             logger.exception("Ошибка в получении пункта из базы данных")
#
#
# def execute_query(connection, query: str, *args):
#     'Функция выполняет запрос к базе данных'
...
# def list_directions():
#     'Функция возвращает список направлений'
#     return execute_query(get_db_connection(), 'select title from points_direction')
#
#
# def list_pzu_in_direction(direction: str):
#     'Функция возвращает список ПЗУ в направлении'
#     direction_id = \
#     execute_query(get_db_connection(), 'select id from points_direction where title = %s;', direction)[0][0]
#     result_request = execute_query(get_db_connection(),
#                                    'select abbreviation from points_location where direction_id = %s;', direction_id)
#     result = [i[0] for i in result_request]
#     return result
#
#
# def list_pzu():
#     'Функция возвращает список ПЗУ'
#     result_request = execute_query(get_db_connection(), 'select abbreviation from points_location')
#     result = [i[0] for i in result_request]
#     return result
